...
/// FIXED: Properly converts local CDT times to UTC.
/// </summary>
public class RecurrenceService
{
    ...
    /// FIXED: Converts local CDT times to UTC properly.
    /// </summary>
    private async Task<int> GenerateInstancesForPatternAsync(RecurrencePattern pattern)
    {
        ...
        while (currentDate.HasValue && currentDate.Value <= endDate)
        {
            ...
            foreach (var patternDoor in pattern.Doors)
            {
                ...
                // FIX: Convert local CDT times to UTC
                // e.g., 8:00 AM CDT -> 13:00 UTC
                var localStart = currentDate.Value.Date.Add(unlockTime);
                var localEnd = currentDate.Value.Date.Add(lockTime);
                
                var schedule = new UnlockSchedule
                {
                    DoorID = patternDoor.DoorID,
                    ScheduleName = pattern.EventName,
                    StartTime = TimeZoneInfo.ConvertTimeToUtc(localStart, CstZone),
                    EndTime = TimeZoneInfo.ConvertTimeToUtc(localEnd, CstZone),
                    Source = "Recurring",
