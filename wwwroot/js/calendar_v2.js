...
// Change event filter from dropdown
function changeEventFilter() {
    ...
    // Re-render calendar with new filter applied
    ...
    const events = Object.values(eventGroups)
        .filter(group => {
            ...
            if (eventFilter === 'special') {
                ...
                return isRecurring; // Only show recurring (weekly) events
            }
            return true; // 'all' - show all events
        })
        .map(group => {
        ...
        // FIX TIMEZONE DISPLAY: API returns all times with "Z", but recurring events are stored in CST (not UTC)
        // Strip Z for recurring events (treat as local CST), keep Z for regular events (convert UTC to CST)
        let startTime = group.unlockTime;
        let endTime = group.lockTime;
        
        if (isRecurring) {
            // Recurring events: times are already in CST, strip the Z to prevent double conversion
            startTime = startTime.replace(/Z$/, '');
            endTime = endTime.replace(/Z$/, '');
        }
        // Regular events: keep Z, browser will convert UTC to local time automatically
