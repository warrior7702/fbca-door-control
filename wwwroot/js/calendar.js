...
let doorsByBuilding = {};
let showRecurringSchedules = true; // Filter state for recurring schedules
...
// Toggle recurring schedule visibility
function toggleRecurringFilter() {
    showRecurringSchedules = !showRecurringSchedules;
    
    // Update button text
    const filterBtn = document.getElementById('filterRecurringBtn');
    if (filterBtn) {
        filterBtn.innerHTML = showRecurringSchedules ? 
            '<span class="btn-icon">📅</span> Show Special Events Only' : 
            '<span class="btn-icon">📅</span> Show All Events';
    }
...
function initializeCalendar() {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventContent: function(arg) {
            ...
            const dotColor = statusColors[status] || statusColors['Pending'];
            ...
            return {
                html: `<div class="fc-event-main-frame" title="${tooltip}">
                    <div class="fc-event-title-container">
                        <span class="event-status-dot" style="background-color: ${dotColor};"></span>
...
function renderCalendarEvents() {
    ...
    const events = Object.values(eventGroups)
        .filter(group => {
            // Apply recurring filter if active
            if (!showRecurringSchedules) {
                const isRecurring = group.schedules.some(s => s.isRecurring === true);
                return !isRecurring; // Only show non-recurring (special) events
            }
        ...
        // All times from API are now proper UTC (backend fixed)
        // FullCalendar will convert UTC to local time automatically
...
function populateDoorDropdowns() {
    // Get all building filter dropdowns
    const wadeSelect = document.getElementById('doorFilterWade');
    const mainChurchSelect = document.getElementById('doorFilterMainChurch');
    const studentCenterSelect = document.getElementById('doorFilterStudentCenter');
    const pcbSelect = document.getElementById('doorFilterPCB');

    // Guard: if any dropdown is missing, skip population (DOM not ready or element removed)
    if (!wadeSelect || !mainChurchSelect || !studentCenterSelect || !pcbSelect) {
        console.warn('Door filter dropdowns not found in DOM, skipping populateDoorDropdowns');
        return;
    }

    // Clear all building filter dropdowns
    wadeSelect.innerHTML = '<option value="">Select door...</option>';
    mainChurchSelect.innerHTML = '<option value="">Select door...</option>';
    studentCenterSelect.innerHTML = '<option value="">Select door...</option>';
    pcbSelect.innerHTML = '<option value="">Select door...</option>';
...
function populateBuildingDropdown(selectId, doors) {
    ...
    // Guard: if element doesn't exist, skip
    if (!select) {
        console.warn(`Dropdown ${selectId} not found in DOM, skipping populateBuildingDropdown`);
        return;
    }

    // Guard: if doors array is missing or empty, skip
    if (!doors || !Array.isArray(doors) || doors.length === 0) {
        console.warn(`No doors provided for ${selectId}, skipping population`);
        return;
    }
...
function filterDoors(building) {
    ...
    // Get all dropdown elements with guards
    const wadeSelect = document.getElementById('doorFilterWade');
    const mainChurchSelect = document.getElementById('doorFilterMainChurch');
    const studentCenterSelect = document.getElementById('doorFilterStudentCenter');
    const pcbSelect = document.getElementById('doorFilterPCB');

    // Guard: if dropdowns aren't ready, bail out
    if (!wadeSelect || !mainChurchSelect || !studentCenterSelect || !pcbSelect) {
        console.warn('Door filter dropdowns not found, cannot filter');
        return;
    }

    if (building === 'wade') {
        selectedDoorId = parseInt(wadeSelect.value);
        // Clear other dropdowns
        mainChurchSelect.value = '';
        studentCenterSelect.value = '';
        pcbSelect.value = '';
    } else if (building === 'mainChurch') {
        selectedDoorId = parseInt(mainChurchSelect.value);
        wadeSelect.value = '';
        studentCenterSelect.value = '';
        pcbSelect.value = '';
    } else if (building === 'studentCenter') {
        selectedDoorId = parseInt(studentCenterSelect.value);
        wadeSelect.value = '';
        mainChurchSelect.value = '';
        pcbSelect.value = '';
    } else if (building === 'pcb') {
        selectedDoorId = parseInt(pcbSelect.value);
        wadeSelect.value = '';
        mainChurchSelect.value = '';
        studentCenterSelect.value = '';
    }
...
function updateStats() {
    const totalDoorsEl = document.getElementById('statTotalDoors');
    const activeSchedulesEl = document.getElementById('statActiveSchedules');
    
    if (totalDoorsEl) {
        totalDoorsEl.textContent = allDoors.length;
    }
    ...
    if (activeSchedulesEl) {
        activeSchedulesEl.textContent = activeSchedules.length;
    }
}
